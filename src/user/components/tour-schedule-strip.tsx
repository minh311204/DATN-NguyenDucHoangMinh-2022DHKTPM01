...
const CHIP_LAYOUT =
  "tour-date-chip px-1.5 py-0.5 text-[11px] font-medium tabular-nums shrink-0 scroll-ml-0 snap-start sm:text-xs";
...
export function TourScheduleStrip({ schedules, tourId }: Props) {
  ...
  if (dayItems.length === 0) {
    return (
      <Link
        ...
        scroll={false}
        className="flex items-center gap-1.5 rounded-md px-0.5 py-0.5 text-[11px] text-stone-500 outline-offset-2 transition hover:bg-stone-50 sm:text-xs"
      >
        <Calendar className="h-3.5 w-3.5 shrink-0 text-stone-400" aria-hidden />
        <span className="font-medium text-stone-700">Ngày khởi hành:</span>
  ...
  return (
    <div
      className="-mx-0.5 flex min-w-0 items-center gap-1 rounded-md px-0.5 py-0.5"
      onClick={(ev) => {
    ...
    >
      ...
      <button
        ...
        className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-stone-200 bg-stone-100 text-stone-800 shadow-sm transition ${
          canScrollLeft
